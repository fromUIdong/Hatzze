...
INDICATOR_META = {
    ...
    "description_beginner": "주가가 급등하면 매수 사이드카, 급락하면 매도 사이드카·서킷브레이커(CB)가 발동됩니다. 최근 한 달 어느 쪽 안전장치가 더 자주 걸렸는지로 시장이 달아올랐는지 식었는지를 읽습니다",
    # 2026-07-20 공식 교체로 raw_value 가 건수 차이 → 0~1 비중이 됐다. 단위를 "건"으로
    # 두면 공용 표시 함수를 타는 경로에서 "0.30건"이 나온다(카드는 전용 렌더라 안 보였다).
    "unit": "비중",
}
