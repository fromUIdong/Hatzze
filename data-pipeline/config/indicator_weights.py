...
쌓이면 재조정. ※ put_call_ratio는 옵션 API 승인(2026-07-20)으로 실데이터 242영업일을
확보해 같은 날 점수에 편입했다(2.5). 임계값 0.50·direction=low 는 중앙값이 상온(53%)에
놓이고 Hit 도달이 4.1%가 되도록 실측 분포에서 잡았다 — market_actions_30d(4.3%)와 같은
엄격도다. 편입으로 weight_sum 이 48.5 → 51.0 이 된다.
"""
...
INDICATOR_WEIGHTS = {
    ...
    "put_call_ratio": 2.5,          # 옵션 콜 쏠림(상승 베팅 과열)
