...
INDICATOR_THRESHOLDS = {
    ...
    #
    # 2026-07-24: 두 지표(dcinside·news_sentiment) 모두 **3일 풀링**을 켰다(common.details의
    # pooled_sentiment_details). raw_value 가 이제 하루치가 아니라 최근 3일 합산 비율이라
    # 분포가 좁아진다. 아래 floor/ceiling 은 **풀링 이전의 하루치 분포**로 07-23에 맞춘 값이라
    # 엄밀히는 다시 잡아야 하는데, 지금은 원자료(일별 건수)가 07-21~24 나흘치뿐이라 풀링
    # 분포를 신뢰성 있게 뽑을 수 없다. 풀링값이 ~30일 쌓이면(대략 8월 말) 그때 실측 분포로
    # 재보정할 것 — 그 전까지는 눈금이 다소 헐거워 중앙에 몰릴 수 있으나(=포화가 덜함) froth
    # 게이지엔 오히려 안전한 쪽이다. 재보정 시 backtest/absolute.py 로 SCORE_DISPLAY_ANCHORS도
    # 함께 확인. (눈금을 표본 유난한 해에 맞추면 영구히 0에 붙는 함정은 아래 kospi_speed_60d
    # 주석 참고.)
